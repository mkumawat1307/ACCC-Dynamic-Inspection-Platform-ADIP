...
The Jest suite runs via jest-expo (69 suites / 808 tests) against in-memory, path-aware mocks (`__mocks__/expo-sqlite.ts`, `__mocks__/expo-file-system.ts`) with per-directory coverage thresholds; test fixtures must use distinct DB paths/names to preserve isolation.
