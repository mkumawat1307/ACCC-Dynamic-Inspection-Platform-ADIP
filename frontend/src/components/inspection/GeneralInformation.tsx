...
// Merges the live (typed, unsaved) form values with the DB-persisted values.
// A non-empty live value wins; otherwise the persisted value is kept. Used when
// a draft-triggered re-init must not clobber fields the user already typed.
function mergeCreateValues(
  ...
  for (const key of keys) {
    merged[key] =
      (live[key] ?? "").trim() !== "" ? (live[key] ?? "") : (saved[key] ?? "");
  }
  return merged;
}
...
>(
  (
    { ensureDraft, releaseAbandonedDraft, existing = false, onDataChanged },
    ref,
  ) => {
    const {
    ...
    const renamePromptResolverRef = useRef<
      ((decision: IdentityRenameDecision) => void) | null
    >(null);
    ...
    async function init() {
      ...
      try {
        ...
        if (!templateId) {
          ...
          const defaultTemplate = await db.getFirstAsync<{
            TemplateID: number;
          }>(
        ...
        const savedValues = await loadInspectionValues(
          loadedFields,
          projectData,
        );
        ...
        let hasTypedPoleId = (valuesRef.current.pole_id ?? "").trim() !== "";

        // CREATE remount recovery: React Native Paper's List.Accordion unmounts this
        // component when its section is collapsed, destroying the local values /
        // valuesRef copy of the typed Site ID. The SQLite pole_id is intentionally
        // empty during CREATE entry (Option-B), so the surviving copy of the typed
        // Site ID is the module-scoped live overlay (InspectionLiveValues). It is
        // reset on screen entry, screen unmount, and "Create New" — never on a
        // section collapse — and the stale-load identity guard above already bound
        // this init to the still-requested inspectionId. On a CREATE remount whose
        // database is still empty, restore that live value as the current typed Site
        // ID. It is merged through mergeCreateValues and used to seed context, but
        // NEVER written to SQLite (Save remains the single commit boundary).
        let livePoleId = "";
        if (
          !existing &&
          inspectionId != null &&
          ...
          const poleField = loadedFields.find((f) => f.FieldKey === "pole_id");
          const live = InspectionLiveValues.getLiveFieldValues();
          if (poleField && live) {
            livePoleId = (live.get(poleField.FieldID) ?? "").trim();
          }
          if (livePoleId !== "") {
            hasTypedPoleId = true;
          }
        ...
        if (savedPoleId !== "" || livePoleId !== "") {
          setFormUnlocked(true);
        }

        // Merge against the latest committed values (valuesRef), not the render-
        // captured closure value, so a draft-triggered re-init does not clobber
        // fields the user already typed but not yet saved (e.g. Block).
        const effectiveValues =
          !savedPoleId && hasTypedPoleId
            ? mergeCreateValues(
                livePoleId !== ""
                  ? { ...valuesRef.current, pole_id: livePoleId }
                  : valuesRef.current,
                savedValues,
              )
            : savedValues;
        ...
        if (savedPoleId !== "") {
          ...
          setPoleId(livePoleId);
        } else if (!hasTypedPoleId) {
          setPoleId("");
        }
        ...
        if (!existing && inspectionId != null) {
          // Pole is never committed during CREATE entry — it stays in React
          // state only and is committed by checkIdentityBeforeSave at Save.
          // Seeding poleId as "" prevents a partial Site ID from becoming
          // the authoritative persisted identity.
          persistedIdentityRef.current = {
            district: effectiveValues.district ?? "",
            block: effectiveValues.block?.trim() ?? "",
            poleId: "",
          };
        }
    ...
    async function loadFields(templateId?: number) {
      try {
        const data = await InspectionRepository.getFieldsByKey(
          "general_information",
          templateId,
        );
        setFields(data);
    ...
    async function handlePoleIdSave(
      ...
      const run = async () => {
        ...
        try {
          ...
          // For CREATE, the draft exists but the Site ID stays in React state only.
          // It will be committed to DB by checkIdentityBeforeSave at Save time.
          // This prevents a partial/unsaved Site ID from becoming the draft's
          // authoritative identity, which would clobber the form on re-init.
          if (!existing) {
            return;
          }
    ...
    // On the first CREATE save no old Site ID was ever persisted, so the only
    // record of the identity the captured photos carry is the token embedded in
    // each filename (generateFileName() structure). Derive that identity and
    // return it only when every relevant photo agrees on one pole token; return
    // null when there is no usable token or the tokens conflict (never guess).
    function deriveCreatePhotoIdentity(
      photos: Photo[],
    ): InspectionIdentity | null {
      const identities = photos.map((photo) =>
        extractIdentityFromFileName(photo.FileName),
      );
      if (identities.some((identity) => identity === null)) return null;

      const captured = identities[0]!;
      const hasConflict = identities.some(
        (identity) =>
          cleanPoleToken(identity!.district) !==
            cleanPoleToken(captured.district) ||
          cleanPoleToken(identity!.block) !== cleanPoleToken(captured.block) ||
          cleanPoleToken(identity!.poleId) !== cleanPoleToken(captured.poleId),
      );
      return hasConflict ? null : captured;
    }
    ...
    async function checkIdentityBeforeSave(): Promise<IdentityRenameDecision> {
      ...
      const identityChanged =
        cleanPoleToken(identity.district) !==
          cleanPoleToken(persisted.district) ||
      ...
      if (!existing && persisted.poleId === "") {
        // CREATE deliberately leaves Site ID unpersisted while typing. Recover the
        // captured identity from the normalized filename tokens. If any photo is
        // tokenless or photos disagree, there is insufficient evidence to choose
        // an old identity, so do not guess or rename files.
        const captured = deriveCreatePhotoIdentity(photos);
        if (!captured) return { type: "no-rename" };

        const capturedIdentityChanged =
          cleanPoleToken(captured.block) !== cleanPoleToken(identity.block) ||
          cleanPoleToken(captured.poleId) !== cleanPoleToken(identity.poleId);
        if (!capturedIdentityChanged) {
          persistedIdentityRef.current = identity;
          return { type: "no-rename" };
        }

        return new Promise((resolve) => {
          renamePromptResolverRef.current = resolve;
          setRenamePrompt({
            oldIdentity: captured,
    ...
    useImperativeHandle<GeneralInformationHandle, GeneralInformationHandle>(
      ref,
      () => ({
        getPoleId() {
    ...
    return (
      <View>
            ...
            <FieldRenderer
              ...
              options={
                field.FieldKey === "division" && contextProject?.DivisionName
                  ? [
                      {
                        label: contextProject.DivisionName,
                        value: contextProject.DivisionName,
                      },
                    ]
                  : field.FieldKey === "district" &&
                      contextProject?.DistrictName
                    ? [
                        {
                          label: contextProject.DistrictName,
                          value: contextProject.DistrictName,
                        },
                      ]
                    : []
              }
        ...
        <PoleRenameConfirmDialog
          ...
          oldIdentity={
            renamePrompt?.oldIdentity ?? { district: "", block: "", poleId: "" }
          }
          newIdentity={
            renamePrompt?.newIdentity ?? { district: "", block: "", poleId: "" }
          }
          ...
          onConfirm={async (renameFiles, updateReports) => {
            ...
            if (resolve)
              resolve({ type: "proceed", renameFiles, updateReports });
            onDataChanged?.();
          }}
        />
      </View>
