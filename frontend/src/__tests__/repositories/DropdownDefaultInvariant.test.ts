...
function createMockDb() {
  ...
  const runAsyncFn = jest.fn().mockImplementation(async (sql: string, params: unknown[]) => {
    ...
    if (sqlUpper.includes("UPDATE FIELDOPTIONS") && sqlUpper.includes("ISACTIVE = 0") && sqlUpper.includes("OPTIONID = ?")) {
      const id = params[0] as number;
      fieldOptions = fieldOptions.map((o) => (o.OptionID === id ? { ...o, IsActive: 0 } : o));
      return { changes: 1 };
    }
    ...
    if (sqlUpper.includes("DELETE FROM FIELDOPTIONS")) {
      ...
      fieldOptions = id === undefined ? [] : fieldOptions.filter((o) => o.OptionID !== id);
      return { changes: 1 };
    }

    if (sqlUpper.includes("DELETE FROM DEVICEOPTIONS")) {
      const id = params?.[0] as number | undefined;
      deviceOptions = id === undefined ? [] : deviceOptions.filter((o) => o.OptionID !== id);
      return { changes: 1 };
    }
...
describe("Dropdown default invariant: COUNT(IsDefault=1) <= 1 per dropdown", () => {
  ...
  describe("Option deletion default integrity", () => {
    it("CASE A: deleting the current default leaves no default (default = NONE)", async () => {
      const { FieldOptionRepository } = require("@/src/database/repositories/FieldOptionRepository");

      mockDb.getFirstAsync.mockResolvedValue({ Max: 0 });

      const yesId = await FieldOptionRepository.create({ FieldID: 1, OptionLabel: "yes", OptionValue: "yes", IsDefault: 1 });
      const noId = await FieldOptionRepository.create({ FieldID: 1, OptionLabel: "no", OptionValue: "no", IsDefault: 0 });

      expect(mockDb.getFieldOptions().find((o) => o.OptionID === yesId)?.IsDefault).toBe(1);

      await FieldOptionRepository.hardDelete(yesId);

      const remaining = mockDb.getFieldOptions();
      expect(remaining.some((o) => o.OptionID === yesId)).toBe(false);
      // The remaining "no" option must NOT have silently become the default.
      expect(remaining.find((o) => o.OptionID === noId)?.IsDefault).toBe(0);
      assertFieldInvariant(mockDb);
    });

    it("CASE B: deleting a non-default leaves the existing default unchanged", async () => {
      const { FieldOptionRepository } = require("@/src/database/repositories/FieldOptionRepository");
      ...
      await FieldOptionRepository.hardDelete(noId);

      expect(mockDb.getFieldOptions().find((o) => o.OptionID === yesId)?.IsDefault).toBe(1);
      assertFieldInvariant(mockDb);
    });

    it("CASE C: deactivating the current default leaves no active default (default = NONE)", async () => {
      const { FieldOptionRepository } = require("@/src/database/repositories/FieldOptionRepository");

      mockDb.getFirstAsync.mockResolvedValue({ Max: 0 });

      const yesId = await FieldOptionRepository.create({ FieldID: 1, OptionLabel: "yes", OptionValue: "yes", IsDefault: 1 });
      await FieldOptionRepository.create({ FieldID: 1, OptionLabel: "no", OptionValue: "no", IsDefault: 0 });

      // Soft delete (IsActive = 0) the current default.
      await FieldOptionRepository.delete(yesId);

      // Option queries filter IsActive = 1, so the deactivated default is no longer a candidate.
      const activeOptions = mockDb.getFieldOptions().filter((o) => o.IsActive === 1);
      expect(activeOptions.some((o) => o.IsDefault === 1)).toBe(false);
    });

    it("CASE E: deleting then recreating the same label does NOT inherit the old default", async () => {
      const { FieldOptionRepository } = require("@/src/database/repositories/FieldOptionRepository");

      mockDb.getFirstAsync.mockResolvedValue({ Max: 0 });

      const oldYesId = await FieldOptionRepository.create({ FieldID: 1, OptionLabel: "yes", OptionValue: "yes", IsDefault: 1 });
      await FieldOptionRepository.create({ FieldID: 1, OptionLabel: "no", OptionValue: "no", IsDefault: 0 });

      await FieldOptionRepository.hardDelete(oldYesId);

      // Recreate a NEW "yes" option with the same label but without explicitly requesting a default.
      const newYesId = await FieldOptionRepository.create({ FieldID: 1, OptionLabel: "yes", OptionValue: "yes", IsDefault: 0 });

      expect(newYesId).not.toBe(oldYesId);
      expect(mockDb.getFieldOptions().find((o) => o.OptionID === newYesId)?.IsDefault).toBe(0);
      assertFieldInvariant(mockDb);
    });
  });
});
