...
import { LOCKED_SECTION_KEYS } from "@/src/database/seeds/factory-config";
...
describe("SmartCardGenerator - card generation", () => {
  ...
  it("returns empty array for a field in the locked remarks section", () => {
    const cards = SmartCardGenerator.generateCardsForField(
      field({ FieldKey: "remarks", FieldName: "Remarks", FieldType: "multiline", SectionKey: "remarks" }),
      1
    );
    expect(cards).toHaveLength(0);
  });

  it("returns empty array for a field in any locked section (general_information, photos)", () => {
    const general = SmartCardGenerator.generateCardsForField(
      field({ FieldKey: "inspection_date", FieldName: "Date", FieldType: "date_auto", SectionKey: "general_information" }),
      1
    );
    const photos = SmartCardGenerator.generateCardsForField(
      field({ FieldKey: "photo_path", FieldName: "Photo", FieldType: "text", SectionKey: "photos" }),
      1
    );
  ...
  it("still generates cards for fields in non-locked sections and for device fields", () => {
    const pole = SmartCardGenerator.generateCardsForField(
      field({ FieldKey: "pole_status", FieldName: "Pole Status", FieldType: "dropdown", SectionKey: "pole_structure" }),
      1
    );
    expect(pole).toHaveLength(2);

    const device = SmartCardGenerator.generateCardsForField(
...
describe("SmartCardGenerator - getFormFields", () => {
  ...
  it("filters out fields in locked sections and no longer hardcodes remarks in SQL", async () => {
    mockDb.getAllAsync
      .mockResolvedValueOnce([
        { FieldID: 1, FieldKey: "remarks", FieldName: "Remarks", FieldType: "multiline", SectionKey: "remarks" },
      ])
      .mockResolvedValueOnce([]);
    ...
    expect(sql).toContain("s.SectionKey");
    expect(sql).not.toContain("FieldKey != 'remarks'");
  });

  it("excludes fields in the general_information, remarks, and photos sections", async () => {
    mockDb.getAllAsync
      .mockResolvedValueOnce([
        { FieldID: 1, FieldKey: "inspection_date", FieldName: "Date", FieldType: "date_auto", SectionKey: "general_information" },
        { FieldID: 2, FieldKey: "remarks", FieldName: "Remarks", FieldType: "multiline", SectionKey: "remarks" },
        { FieldID: 3, FieldKey: "photo_path", FieldName: "Photo", FieldType: "text", SectionKey: "photos" },
        { FieldID: 4, FieldKey: "pole_status", FieldName: "Pole Status", FieldType: "dropdown", SectionKey: "pole_structure" },
      ])
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([]);
    ...
    expect(keys).toEqual(["pole_status"]);
    expect(fields.length).toBeLessThan(4);
  });

  it("tracks SectionKey on the returned inspection fields", async () => {
    mockDb.getAllAsync
      .mockResolvedValueOnce([
        { FieldID: 4, FieldKey: "pole_status", FieldName: "Pole Status", FieldType: "dropdown", SectionKey: "pole_structure" },
      ])
    ...
    expect(fields[0].SectionKey).toBe("pole_structure");
  });

  it("derives exclusions from the shared factory-config locked-section set", async () => {
    mockDb.getAllAsync.mockResolvedValue(
      Array.from(LOCKED_SECTION_KEYS).map((key, i) => ({
        FieldID: i + 1,
        FieldKey: `field_${key}`,
        FieldName: key,
        FieldType: "text",
...
describe("SmartCardGenerator - getAvailableFields", () => {
  ...
  it("hides covered fields, covered counts, and plain text fields", async () => {
    mockDb.getAllAsync
    ...
    expect(keys).not.toContain("camera_status");
    expect(keys).toEqual([]);
  });
  ...
  it("never offers uncovered fields that live in locked sections", async () => {
    mockDb.getAllAsync
      .mockResolvedValueOnce(DEFAULT_CARDS)
      .mockResolvedValueOnce([
        { FieldID: 1, FieldKey: "pole_avail", FieldName: "Pole Availability", FieldType: "dropdown" },
        { FieldID: 2, FieldKey: "inspection_date", FieldName: "Date", FieldType: "date_auto", SectionKey: "general_information" },
        { FieldID: 3, FieldKey: "pole_status", FieldName: "Pole Status", FieldType: "dropdown", SectionKey: "pole_structure" },
      ])
      .mockResolvedValue([]);
    ...
    const keys = available.map((f) => f.FieldKey);
    expect(keys).not.toContain("inspection_date");
    expect(keys).toContain("pole_status");
  });
});

describe("SmartCardGenerator - Add Card picker filtering", () => {
  let mockDb: ReturnType<typeof createMockDb>;
  ...
  it("isDeviceCountField recognizes {device_type}_count keys against real device types", () => {
    const camera = field({ FieldKey: "camera_count", FieldType: "number" });
    expect(SmartCardGenerator.isDeviceCountField(camera, ["Camera", "Switch"])).toBe(true);
    const switchField = field({ FieldKey: "switch_count", FieldType: "number" });
    expect(SmartCardGenerator.isDeviceCountField(switchField, ["Camera", "Switch"])).toBe(true);
  });

  it("isDeviceCountField matches custom device types without hardcoded names", () => {
    const nvr = field({ FieldKey: "nvr_count", FieldType: "number" });
    expect(SmartCardGenerator.isDeviceCountField(nvr, ["NVR"])).toBe(true);
    const gate = field({ FieldKey: "gate_controller_count", FieldType: "number" });
    expect(SmartCardGenerator.isDeviceCountField(gate, ["Gate Controller"])).toBe(true);
  });

  it("isDeviceCountField rejects normal numbers, plain fields, and orphan counts", () => {
    const cable = field({ FieldKey: "cable_length", FieldType: "number" });
    expect(SmartCardGenerator.isDeviceCountField(cable, ["Camera"])).toBe(false);
    const voltage = field({ FieldKey: "earthing_voltage", FieldType: "number" });
    expect(SmartCardGenerator.isDeviceCountField(voltage, ["Camera", "Switch"])).toBe(false);
    const orphan = field({ FieldKey: "camera_count", FieldType: "number" });
    expect(SmartCardGenerator.isDeviceCountField(orphan, ["Switch"])).toBe(false);
  });

  it("isPickerEligible removes plain text and non-device-count numbers, keeps the rest", () => {
    const types = ["Camera", "Switch"];
    expect(SmartCardGenerator.isPickerEligible(field({ FieldKey: "cable_length", FieldType: "number" }), types)).toBe(false);
    expect(SmartCardGenerator.isPickerEligible(field({ FieldKey: "meter_serial", FieldType: "text" }), types)).toBe(false);
    expect(SmartCardGenerator.isPickerEligible(field({ FieldKey: "notes", FieldType: "multiline" }), types)).toBe(false);
    expect(SmartCardGenerator.isPickerEligible(field({ FieldKey: "camera_count", FieldType: "number" }), types)).toBe(true);
    expect(SmartCardGenerator.isPickerEligible(field({ FieldKey: "pole_avail", FieldType: "dropdown" }), types)).toBe(true);
    expect(SmartCardGenerator.isPickerEligible(field({ FieldKey: "pole_ground", FieldType: "checkbox" }), types)).toBe(true);
    expect(SmartCardGenerator.isPickerEligible(deviceField({ FieldType: "text" }), types)).toBe(true);
  });

  it("keeps device-count numbers, dropdowns and checkboxes; drops plain text and plain numbers from the pool", async () => {
    const FIELD_POOL = [
      { FieldID: 1, FieldKey: "cable_length", FieldName: "Power Cable Length", FieldType: "number" },
      { FieldID: 2, FieldKey: "earthing_voltage", FieldName: "Earthing Voltage", FieldType: "number" },
      { FieldID: 3, FieldKey: "meter_serial", FieldName: "Meter Serial Number", FieldType: "text" },
      { FieldID: 4, FieldKey: "camera_count", FieldName: "Camera Count", FieldType: "number" },
      { FieldID: 5, FieldKey: "switch_count", FieldName: "Switch Count", FieldType: "number" },
      { FieldID: 6, FieldKey: "pole_avail", FieldName: "Pole Availability", FieldType: "dropdown" },
      { FieldID: 7, FieldKey: "pole_ground", FieldName: "Pole Ground", FieldType: "checkbox" },
    ];

    mockDb.getAllAsync
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce(FIELD_POOL);
    for (let i = 0; i < FIELD_POOL.length; i++) {
      mockDb.getAllAsync.mockResolvedValueOnce([]);
    }
    mockDb.getAllAsync
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([{ DeviceType: "Camera" }, { DeviceType: "Switch" }])
      .mockResolvedValue([]);

    const available = await SmartCardGenerator.getAvailableFields(1);

    const keys = available.map((f) => f.FieldKey).sort();
    expect(keys).toEqual(["camera_count", "pole_avail", "pole_ground", "switch_count"]);
  });
});
