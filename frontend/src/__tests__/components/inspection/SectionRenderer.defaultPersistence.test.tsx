...
describe("SectionRenderer default selection persistence — Phase 7B regression", () => {
  ...
  describe("existing inspection — current defaults are NEVER shown or staged (saved-only display)", () => {
    ...
    it("existing + no saved value + multiple fields with defaults: none are auto-persisted", async () => {
      const second = { ...dropdownField, FieldID: 13, FieldKey: "cable_type" };
      fieldRepo.getFieldsBySection.mockResolvedValue([dropdownField, second]);
      fieldRepo.getFieldOptions.mockResolvedValue(createDropdownOptions());
    ...
    it("existing + saved value: persisted value remains authoritative over default", async () => {
      fieldRepo.getFieldsBySection.mockResolvedValue([dropdownField]);
      fieldRepo.getFieldOptions.mockResolvedValue(createDropdownOptions());
      fieldRepo.getFieldOptionsBySection.mockResolvedValue(new Map([[10, createDropdownOptions()]]));
      valueRepo.getValuesByInspection.mockResolvedValue([
        {
          ValueID: 1, InspectionID: 42, FieldID: 10, FieldValue: "Overhead",
          CreatedAt: "", UpdatedAt: "",
        },
      ]);
      ...
      expect(valueRepo.saveValue).not.toHaveBeenCalled();
      const input = tree.root.findByProps({ testID: "field-power_cable_status" });
      expect(input.props.value).toBe("Overhead");
    });

    it("existing: opening/cancelling produces zero InspectionValues writes (no inserts)", async () => {
      fieldRepo.getFieldsBySection.mockResolvedValue([dropdownField]);
      fieldRepo.getFieldOptions.mockResolvedValue(createDropdownOptions());
      fieldRepo.getFieldOptionsBySection.mockResolvedValue(new Map([[10, createDropdownOptions()]]));
      valueRepo.getValuesByInspection.mockResolvedValue([]);
