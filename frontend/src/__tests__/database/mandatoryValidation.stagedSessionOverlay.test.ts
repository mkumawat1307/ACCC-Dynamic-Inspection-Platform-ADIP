...
jest.mock("@/src/utils/downloadStorage", () => ({
  downloadStorage: {
    ...
    writeBase64: jest
      .fn()
      .mockResolvedValue("content://mock/exported/file.xlsx"),
...
describe("mandatory validation overlays InspectionEditSession-staged values", () => {
  ...
  async function openSeededProject(
    path = PROJECT,
  ): Promise<{ db: SQLiteDatabase }> {
    const dbModule =
      require("@/src/database/db") as typeof import("@/src/database/db");
  ...
  async function createInspection(date: string): Promise<number> {
    const { InspectionRepository } =
      require("@/src/database/repositories/InspectionRepository") as typeof import("@/src/database/repositories/InspectionRepository");
  ...
  async function seedAllRequiredExcept(
    ...
    const required = await db.getAllAsync<{
      FieldID: number;
      FieldKey: string;
      FieldType: string;
  ...
  it("field-level lifecycle: deleted standard mandatory field -> new inspection -> reset restores same FieldID -> staged selection validated without explicit param", async () => {
    ...
    const { ResetRepository } =
      require("@/src/database/repositories/ResetRepository") as typeof import("@/src/database/repositories/ResetRepository");
    const { InspectionRepository } =
      require("@/src/database/repositories/InspectionRepository") as typeof import("@/src/database/repositories/InspectionRepository");
    const InspectionValueRepository =
      require("@/src/database/repositories/InspectionValueRepository")
        .default as typeof import("@/src/database/repositories/InspectionValueRepository").default;
    const { InspectionEditSession } =
      require("@/src/database/repositories/InspectionEditSession") as typeof import("@/src/database/repositories/InspectionEditSession");

    const foundationId = await getFieldId(db, "foundation_cond");
    expect(foundationId).not.toBeNull();

    await db.runAsync(
      "UPDATE InspectionFields SET IsActive = 0 WHERE FieldID = ?",
      [foundationId!],
    );
    ...
    expect(
      (await InspectionRepository.validateInspection(inspectionId)).valid,
    ).toBe(true);

    await ResetRepository.performReset();
    const restored = await db.getFirstAsync<{
      FieldID: number;
      IsActive: number;
    }>(
    ...
    expect(
      stagedSeen.missingFields.filter((f) => f === "Foundation Condition"),
    ).toEqual([]);

    const committed = await InspectionEditSession.commit();
    expect(committed).toBe(true);
    expect(
      (await InspectionValueRepository.getValue(inspectionId, foundationId!))
        ?.FieldValue,
    ).toBe("Acceptable");
  ...
  it("option-level lifecycle: deleted mandatory option -> new inspection -> reset restores option in place -> staged restored selection persists", async () => {
    ...
    const { InspectionRepository } =
      require("@/src/database/repositories/InspectionRepository") as typeof import("@/src/database/repositories/InspectionRepository");
    const InspectionFieldRepository: typeof import("@/src/database/repositories/InspectionFieldRepository").default =
    ...
    await db.runAsync(
      "UPDATE FieldOptions SET IsActive = 0 WHERE FieldID = ? AND OptionValue = 'Acceptable'",
      [foundationId!],
    );

    const midInspectionId = await createInspection("2026-09-05");
    const midOptions = await InspectionFieldRepository.getFieldOptionsBySection(
      fieldRow!.SectionID,
      midInspectionId,
    );
    expect(
      (midOptions.get(foundationId!) ?? []).map((o) => o.OptionValue),
    ).not.toContain("Acceptable");

    await ResetRepository.performReset();
    const restoredOption = await db.getFirstAsync<{
      OptionID: number;
      IsActive: number;
    }>(
  ...
  it("device factory lifecycle: deleted device type -> new inspection -> reset -> staged device edit passes validateDeviceMandatory", async () => {
    ...
    const { InspectionEditSession } =
      require("@/src/database/repositories/InspectionEditSession") as typeof import("@/src/database/repositories/InspectionEditSession");
    const DeviceRecordsRepository =
      require("@/src/database/repositories/DeviceRecordsRepository")
        .default as typeof import("@/src/database/repositories/DeviceRecordsRepository").default;
    const DeviceFieldDefsRepository =
      require("@/src/database/repositories/DeviceFieldDefinitionsRepository")
        .default as typeof import("@/src/database/repositories/DeviceFieldDefinitionsRepository").default;

    await db.runAsync(
      "UPDATE DeviceFieldDefinitions SET IsActive = 0 WHERE DeviceType = 'Camera'",
    );
    await db.runAsync(
      "DELETE FROM ProjectDeviceTypes WHERE DeviceType = 'Camera'",
    );
    ...
    await ResetRepository.performReset();
    const reactivated = await db.getAllAsync<{
      FieldName: string;
      IsRequired: number;
      IsActive: number;
    }>(
    ...
    const requiredNames = reactivated
      .filter((d) => d.IsRequired === 1)
      .map((d) => d.FieldName);
    ...
    const dbRecord = (
      await DeviceRecordsRepository.getByInspection(inspectionId, "Camera")
    )[0];
    ...
    expect(
      (await DeviceRecordsRepository.getByInspection(inspectionId, "Camera"))[0]
        .DeviceData,
    ).toBe("{}");
    ...
    expect(
      (await DeviceRecordsRepository.getByInspection(inspectionId, "Camera"))[0]
        .DeviceData,
    ).toBe(JSON.stringify({ CameraType: "PTZ", CameraStatus: "VMS" }));
    expect(
      (await InspectionRepository.validateDeviceMandatory(inspectionId)).valid,
    ).toBe(true);
  });
  ...
  it("device factory non-mandatory: staged optional CameraMake edit passes and persists", async () => {
    ...
    expect(await DeviceFieldDefsRepository.getDeviceTypes()).not.toContain(
      "Camera",
    );
    ...
    expect(JSON.parse(persisted.DeviceData ?? "{}").CameraMake).toBe(
      "Hikvision",
    );
    expect(
      (await InspectionRepository.validateDeviceMandatory(inspectionId)).valid,
  ...
  it("custom section mandatory: deleted custom field -> new inspection -> custom re-added -> staged selection passes without explicit param", async () => {
    ...
    const sectionId = await createCustomSection(
      db,
      "Custom Pylon",
      "custom_pylon",
      90,
    ...
    await db.runAsync(
      "UPDATE InspectionSections SET IsActive = 0 WHERE SectionID = ?",
      [sectionId],
    );
    await db.runAsync(
      "UPDATE InspectionFields SET IsActive = 0 WHERE FieldID = ?",
    ...
    await seedAllRequiredExcept(db, inspectionId, []);
    const midSections = await InspectionRepository.getSections(
      undefined,
      inspectionId,
    );
    expect(
      midSections.some((s: SectionRow) => s.SectionKey === "custom_pylon"),
    ).toBe(false);
    expect(
      (await InspectionRepository.validateInspection(inspectionId)).valid,
    ).toBe(true);

    const restoredSectionId = await createCustomSection(
      db,
      "Custom Pylon",
      "custom_pylon",
      90,
    );
    const restoredFieldId = await createCustomField(
      db,
      restoredSectionId,
      "Pylon Condition Text",
      "pylon_cond_text",
    ...
    expect(stagedSeen.valid).toBe(true);
    expect(
      stagedSeen.missingFields.filter((f) => f === "Pylon Condition Text"),
    ).toEqual([]);

    const committed = await InspectionEditSession.commit();
    expect(committed).toBe(true);
    expect(
      (await InspectionValueRepository.getValue(inspectionId, restoredFieldId))
        ?.FieldValue,
    ).toBe("Side A");
    expect(
      (await InspectionRepository.validateInspection(inspectionId)).valid,
  ...
  it("custom section non-mandatory: staged custom optional value persists without false positives", async () => {
    ...
    expect(
      midSections.some((s: SectionRow) => s.SectionKey === "custom_notes"),
    ).toBe(false);
  ...
  it("custom device mandatory: deleted custom device type -> new inspection -> re-added -> staged record passes validateDeviceMandatory", async () => {
    ...
    const nvrSectionId = await createCustomSection(
      db,
      "NVR Devices",
      "nvr_devices",
      88,
    );
    const nvrCountId = await createCustomField(
      db,
      nvrSectionId,
      "NVR Count",
      "nvr_count",
    ...
    await db.runAsync(
      "UPDATE DeviceFieldDefinitions SET IsActive = 0 WHERE DeviceType = 'NVR'",
    );
    await db.runAsync(
      "DELETE FROM ProjectDeviceTypes WHERE DeviceType = 'NVR'",
    );
    ...
    const withoutStaged =
      await InspectionRepository.validateDeviceMandatory(inspectionId);
    expect(withoutStaged.valid).toBe(false);
    expect(withoutStaged.missingFields).toContain(
      "NVR — NVR Status (Device 1)",
    );
  ...
  it("custom device non-mandatory: staged custom optional value persists without false positives", async () => {
    ...
    const sanSectionId = await createCustomSection(
      db,
      "SAN Devices",
      "san_devices",
      87,
    );
    const sanCountId = await createCustomField(
      db,
      sanSectionId,
      "SAN Count",
      "san_count",
    ...
    await db.runAsync(
      "UPDATE DeviceFieldDefinitions SET IsActive = 0 WHERE DeviceType = 'SAN'",
    );
    ...
    const dbRecord = (
      await DeviceRecordsRepository.getByInspection(inspectionId, "SAN")
    )[0];
    InspectionEditSession.activate(inspectionId);
    await DeviceRecordsRepository.scheduleDeviceRecordSave(
      {
  ...
  it("device count overlay: staged count raises DB threshold and staged records are enforced, then pass once filled", async () => {
    ...
    expect(enforced.missingFields).toContain(
      "Camera — Camera Status (Device 1)",
    );
    ...
    expect(committed).toBe(true);
    expect(
      (await InspectionValueRepository.getValue(inspectionId, cameraCountId!))
        ?.FieldValue,
    ).toBe("1");
    const persisted = (
      await DeviceRecordsRepository.getByInspection(inspectionId, "Camera")
  ...
  it("F1 CREATE Option-B: a live (in-memory) Site ID overlay satisfies section validation while the DB pole_id stays empty, and validation persists nothing", async () => {
    const { db } = await openSeededProject();
    const { InspectionRepository } =
      require("@/src/database/repositories/InspectionRepository") as typeof import("@/src/database/repositories/InspectionRepository");
    const InspectionValueRepository =
    ...
    const inspectionId = await createInspection("2026-09-15");
    await seedAllRequiredExcept(db, inspectionId, ["pole_id"]);
    const poleFieldId = await getFieldId(db, "pole_id");
    expect(poleFieldId).not.toBeNull();

    // Pre-F1 regression: under CREATE Option-B the DB pole_id is intentionally
    // empty until Save commits it, so validation without the live overlay fails
    // with "Site ID" even though the user has already typed it on screen.
    expect(
      (await InspectionRepository.validateInspection(inspectionId)).valid,
    ).toBe(false);
    expect(
    ...
    // F1: the live typing overlay (the value on screen before Save) satisfies
    // the required check without ever touching the database.
    const overlay = new Map([[poleFieldId!, "SIK098/076sik/09845/123"]]);
    const passed = await InspectionRepository.validateInspection(
      inspectionId,
      overlay,
    );
    expect(passed.valid).toBe(true);
    ...
    // Validation itself must never persist the Site ID — Save is the single
    // authoritative CREATE commit boundary (Option-B).
